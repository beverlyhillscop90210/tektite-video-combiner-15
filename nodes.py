...
class TektiteVideoCombiner15:
    ...
    def combine_videos(
        ...
        **kwargs,
        ...
        while time.time() < deadline:
            ...
            raise ValueError(
                f"Tektite Video Combiner 15.0 timed out after {wait_timeout_sec}s while waiting. Last state: {last_state}"
            )
        print(
            f"[Tektite Video Combiner 15.0] Ready {expected_clips}/{expected_clips}. Starting normalize+stitch."
        )
        ...
        try:
            ...
            for idx, unit in enumerate(ordered_units, start=1):
                if unit["kind"] == "video":
                    print(
                        f"[Tektite Video Combiner 15.0] Clip {idx}/{len(ordered_units)} is video: "
                    ...
                    print(
                        f"[Tektite Video Combiner 15.0] Clip {idx}/{len(ordered_units)} is image sequence: "
                    ...
                    print(
                        f"[Tektite Video Combiner 15.0] Clip {idx}/{len(ordered_units)} sequence render done: "
                        f"{seq_video_path}"
                    )
    ...
    def _build_progress_line(
        ...
        *,
        ...
        return (
            f"[Tektite Video Combiner 15.0] Clips ready: {ready_clips}/{expected_clips} | "
    ...
    def _collect_clip_units(
        ...
        for key, value in dynamic_inputs.items():
            ...
            video_paths = [
                p for p in paths
                if self._is_video_file(p) or (os.path.isfile(p) and not self._is_image_file(p))
            ]
            if video_paths:
                ignored_images = [p for p in paths if self._is_image_file(p)]
                if ignored_images:
                    print(
                        f"[Tektite Video Combiner 15.0] clip{slot_index}: "
                        f"using video path(s), ignoring {len(ignored_images)} preview image path(s)"
                    )
                for p in video_paths:
                    ...
                    seq += 1
                per_slot_paths.setdefault(slot_index, []).extend(video_paths)
                continue
    ...
    def _persist_image_tensor_sequence(self, tensor: torch.Tensor, *, slot_index: int) -> List[str]:
        ...
        paths: List[str] = []
        print(
            f"[Tektite Video Combiner 15.0] clip{slot_index}: received IMAGE batch with "
    ...
    def _extract_paths_from_value(self, value: Any, _depth: int = 0, _seen: Optional[set] = None) -> List[str]:
        ...
        if hasattr(value, "get_stream_source"):
            try:
                ...
                print(f"[Tektite Video Combiner 15.0] get_stream_source failed: {type(exc).__name__}: {exc}")
        ...
        if hasattr(value, "save_to"):
            try:
                ...
                print(f"[Tektite Video Combiner 15.0] save_to fallback failed: {type(exc).__name__}: {exc}")
    ...
    def _log_unrecognized_clip_input(self, slot_index: int, value: Any) -> None:
        ...
        print(
            f"[Tektite Video Combiner 15.0] clip{slot_index}: input connected but not recognized "
    ...
    def _render_image_sequence_to_video(
        ...
        *,
        ...
        print(
            f"[Tektite Video Combiner 15.0] Rendering image sequence: "
        ...
        print(f"[Tektite Video Combiner 15.0] Image sequence video ready: {out_path}")
        return out_path
    ...
    def _ffmpeg_stitch(
        ...
        *,
        ...
        try:
            ...
            print(
                f"[Tektite Video Combiner 15.0] Normalizing all clips to "
                f"{target_width}x{target_height} at {float(target_fps):.2f} fps"
            )

            # Stage 1: normalize each clip to one duration-preserving frame clock,
            # codec, pixel format, and canvas size.
    ...
    def _mux_audio_track(self, *, video_path: str, audio: Any, overwrite: bool) -> str:
        ...
        out_dir = os.path.dirname(video_path) or folder_paths.get_output_directory()
        base_name, ext = os.path.splitext(os.path.basename(video_path))
        fd, muxed_path = tempfile.mkstemp(prefix=f".{base_name}_mux_", suffix=ext or ".mp4", dir=out_dir)
        os.close(fd)
    ...
    def _safe_remove(self, path: str) -> None:
        ...
        try:
            ...
            print(f"[Tektite Video Combiner 15.0] cleanup ignored for {path}: {type(exc).__name__}: {exc}")
    ...
    def _ensure_writable_output_path(self, resolved_output: str) -> str:
        ...
        print(
            f"[Tektite Video Combiner 15.0] Output directory not writable: {out_dir}. "
...
NODE_CLASS_MAPPINGS = {
    "TektiteVideoCombiner15": TektiteVideoCombiner15,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "TektiteVideoCombiner15": "Tektite Video Combiner 15.0",
}
